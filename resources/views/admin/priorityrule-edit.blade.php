...
<div class="container pb-3">
    ...
    <div class="row">
      <h3 class="col pt-2">@lang('main.priorities.edit_rule_title')</h3>
    </div>
    <hr class="mb-3 mt-0">
    ...
    <div class="editor">
      ...
      <form action="{{ route('priorityrules.update', $rule->id) }}" method="post">
        @csrf
        @method('put')
        ...
        <label>@lang('main.priorities.conditions')</label>
        <div class="bg-slate p-2">
            ...
            @foreach ($rule->conditions as $con)
              <div class="card slate">
                <div class="d-flex" style="padding: .5rem 1rem;">
                  <span class="pr-3"><b>
                    @if ($con->condition=='clients') @lang('main.clients.client')
                    @elseif ($con->condition=='ext_users') @lang('main.incidents.table.creator')
                    @endif
                  </b></span>
                  <span>
                    {{ $con->helper }} 
          ...
          <p id="vacio" hidden>@lang('main.priorities.no_conditions')</p>
        ...
        <div class="form-group mt-3">
          <label for="formControlRange" id="points_text">
          </label>
          <input type="range" id="points" class="form-control-range" name="points" 
            min="0" max="100" step="5" value="{{ $rule->points }}">
        </div>
...
<script>
  ...
  function changeText() {
    var def = '{{ __("main.priorities.priority") }}';
    if ($('#points').val()==0)
      output.innerHTML = def + ' ' + '{{ __("main.priorities.not_setted") }}';
    else
      output.innerHTML = def + ' ' +  $('#points').val();
  }
              ...
              <form action="" id="agregaCondicion">
                ...
                <div class="form-group">
                  ...
                  <select id="valor" name="valor" class="form-control">
                    ...
                    <option value='ext_users'>@lang('main.incidents.table.creator')</option>
                  </select>
                </div>
