...
class DashboardController extends Controller
{
    ...
    {
        $this->filters = $this->getMixedFilters('dashboard_filters');
    ...
    private function getFilteredIncidents($filter, $sla)
    {
        ...
        $incidents = Incident::getIncidentList();

        $incidents = SlaStatusFilter::filter($incidents, $filter, $sla);

        return $incidents->filter($this->incident_filters, $this->filters)
                ->orderBy('id', 'desc')->paginate(15);
        
        //return $incidents->paginate(15);
